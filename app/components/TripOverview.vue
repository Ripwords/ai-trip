...
const accommodationGroups = computed(() => {
  ...
  for (const day of props.sortedDays) {
    if (!day.accommodationName) { current = null; continue; }
    if (current && current.name === day.accommodationName) {
      current.dayNumbers.push(day.dayNumber);
    } else {
      current = { name: day.accommodationName, address: day.accommodationAddress, dayNumbers: [day.dayNumber] };
      groups.push(current);
    }
  }
  return groups;
...
<template>
  <div class="relative">
    ...
    <div class="hidden lg:relative lg:mx-auto lg:block lg:h-[calc(100vh-220px)] lg:overflow-hidden lg:rounded-2xl lg:shadow-lg">
      ...
      >
        <div class="space-y-2.5">
          ...
          <div v-if="upcomingActivities.length > 0" class="ov-card p-4">
            <h3 class="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-terra-500">
            ...
            <div class="mt-2.5 space-y-1">
              ...
              >
                ...
                <span class="ml-auto shrink-0 text-[11px] text-sand-400">D{{ item.dayNumber }}</span>
              </button>
            </div>
          </div>
          ...
          <div class="ov-card p-3">
            <div class="flex items-center justify-between px-1 pb-2">
              <h3 class="ov-section-title text-[11px] font-semibold uppercase tracking-widest">Itinerary</h3>
            ...
            <div class="space-y-px">
              ...
              >
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <div v-if="day.accommodationName" class="day-sub truncate text-[11px] max-w-[160px]">
                ...
                >
                  ...
                  <span class="text-[11px] tabular-nums">{{ weatherByDate.get(day.date)!.tempMax }}°</span>
                </div>
              </button>
            </div>
          </div>
          ...
          <div v-if="accommodationGroups.length > 0" class="ov-card p-4">
            ...
            <div class="mt-2.5 space-y-1.5">
              ...
              >
                <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-forest-50 text-forest-600">
          ...
          <div v-if="Object.keys(activitiesByType).length > 0" class="ov-card p-4">
            ...
            <div class="mt-2.5 space-y-0.5">
              <div v-for="(activities, type) in activitiesByType" :key="type">
                ...
                >
                  <span class="h-2 w-2 shrink-0 rounded-full" :class="typeColors[type as string] || 'bg-sand-400'" />
                  ...
                  <Icon name="lucide:chevron-down" class="h-3 w-3 text-sand-300 transition-transform" :class="{ '-rotate-180': expandedTypes.has(type as string) }" />
                </button>
                <div v-if="expandedTypes.has(type as string)" class="ml-5 border-l border-sand-100 pl-3 pb-1">
                  <div v-for="activity in activities" :key="activity.id" class="flex items-center justify-between py-1.5">
                    <span class="text-xs text-sand-600 truncate">{{ activity.name }}</span>
                    <span v-if="activity.costEstimate" class="shrink-0 text-[11px] tabular-nums text-sand-400">
          ...
          <div v-if="expensesByCategory.length > 0" class="ov-card p-4">
            <h3 class="ov-section-title text-xs font-semibold uppercase tracking-wider">Spending</h3>
            <div class="mt-2.5 space-y-2">
              <div v-for="[category, amount] in expensesByCategory" :key="category" class="flex items-center justify-between">
                <span class="text-xs text-sand-600">{{ formatType(category) }}</span>
                <span class="text-xs font-semibold tabular-nums text-sand-800">{{ formatCurrency(amount) }}</span>
              </div>
              <div class="flex items-center justify-between border-t border-sand-100 pt-2">
                ...
                <span class="text-sm font-display text-sand-900">{{ formatCurrency(totalExpenses) }}</span>
              </div>
            </div>
          </div>
    ...
    <div class="mt-5 space-y-3 lg:hidden">
      ...
      <div class="ov-card p-3">
        <div class="flex items-center justify-between px-1 pb-2">
          <h3 class="ov-section-title text-[11px] font-semibold uppercase tracking-widest">Itinerary</h3>
          <span v-if="selectedDayId" class="text-[11px] text-terra-500 cursor-pointer" @click="selectedDayId = null">Clear</span>
        </div>
        <div class="space-y-px">
          <button
            v-for="day in sortedDays"
          ...
          >
            ...
            <div class="min-w-0 flex-1">
              ...
              <div v-if="day.accommodationName" class="day-sub truncate text-[11px]">{{ day.accommodationName }}</div>
              <div v-else-if="!day.activities.length" class="day-sub text-[11px] italic">Nothing planned</div>
            </div>
            <div v-if="weatherByDate.get(day.date)" class="ov-day-weather flex shrink-0 items-center gap-1">
              <Icon :name="getWeatherInfo(weatherByDate.get(day.date)!.weatherCode).icon" class="h-3.5 w-3.5" />
              <span class="text-[11px] tabular-nums">{{ weatherByDate.get(day.date)!.tempMax }}°</span>
            </div>
          </button>
      ...
      <div v-if="accommodationGroups.length > 0" class="ov-card p-4">
        <h3 class="ov-section-title text-xs font-semibold uppercase tracking-wider">Stays</h3>
        <div class="mt-2.5 space-y-1.5">
          <div v-for="group in accommodationGroups" :key="group.name + group.dayNumbers[0]" class="flex items-center gap-2.5 rounded-lg px-2.5 py-2">
            <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-forest-50 text-forest-600">
              <Icon name="lucide:bed-double" class="h-3.5 w-3.5" />
            </span>
            <div class="min-w-0 flex-1">
...
.ov-toggle:hover { color: #e85d3a; }

/* Day rows — light */
.ov-day-num { background: #f3efe8; color: #6e5c46; }
.ov-day-row:hover .ov-day-num { background: #e85d3a; color: #fff; }
.ov-day-date { color: #3d3328; }
.ov-day-meta { color: #9f8b6f; }
.ov-day-sub { color: #b8a78f; }
.ov-day-weather { color: #9f8b6f; }
.ov-day-row:hover { background: rgba(232, 93, 58, 0.06); }
.ov-day-row-active { background: rgba(232, 93, 58, 0.1); }
.ov-day-num-active { background: #e85d3a !important; color: #fff !important; }
.ov-section-title { color: #9f8b6f; }
...
.dark .ov-toggle:hover { color: #e85d3a; }

/* Day rows — dark */
.dark .ov-day-num { background: rgba(255, 255, 255, 0.08); color: rgba(255, 255, 255, 0.7); }
.dark .ov-day-row:hover .ov-day-num { background: #e85d3a; color: #fff; }
.dark .ov-day-date { color: rgba(255, 255, 255, 0.85); }
.dark .ov-day-meta { color: rgba(255, 255, 255, 0.4); }
.dark .ov-day-sub { color: rgba(255, 255, 255, 0.3); }
.dark .ov-day-weather { color: rgba(255, 255, 255, 0.4); }
.dark .ov-day-row:hover { background: rgba(255, 255, 255, 0.04); }
.dark .ov-day-row-active { background: rgba(232, 93, 58, 0.12); }
.dark .ov-day-num-active { background: #e85d3a !important; color: #fff !important; }
.dark .ov-section-title { color: rgba(255, 255, 255, 0.35); }
