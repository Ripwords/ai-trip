  ...
  <section class="grid gap-3 lg:grid-cols-[minmax(0,1.32fr)_minmax(260px,0.68fr)]">
    ...
    <div class="grid grid-cols-3 gap-1.5 sm:grid-cols-1 sm:gap-3">
      <template v-for="signal in briefing.signals" :key="signal.key">
        ...
        <div
          v-else
          class="flex min-h-11 items-center justify-center gap-1.5 overflow-hidden rounded-full px-2 py-1.5 text-[11px] font-semibold text-sand-900 sm:min-h-[72px] sm:justify-start sm:items-start sm:gap-3 sm:rounded-2xl sm:px-4 sm:py-4 sm:text-sm sm:shadow-sm sm:shadow-sand-900/5"
          :class="signalClass(signal)"
        >
          <span
            class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full sm:h-9 sm:w-9 sm:rounded-xl"
