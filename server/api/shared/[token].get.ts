...
export default defineEventHandler(async (event) => {
  ...
  // Budget and expenses are intentionally omitted for the same reason: a share link
  // can be forwarded to anyone, and trip finances are not public-safe. `currencyCode`
  // is kept only so activity cost estimates render with the right symbol.
  // Note: the PDF export (app/composables/useExportPdf.ts) *does* include a budget
  // section. That is deliberate — a PDF is generated by the owner and sent to a
  // chosen recipient, whereas a share link has no audience control.
